...
public class BookedPage extends javax.swing.JPanel {
    ...
    public BookedPage(Dashboard dashboard) {
        initComponents();
    ...
    private void purchaseButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_purchaseButtonActionPerformed
        ...
        if (codeText.getText().equals("") || jDateChooser1.getDate()==null
            ...
            itemDTO.setItemCode(codeText.getText());
            try {
                ResultSet resultSet = new ItemDAO().getItemName(codeText.getText());
                if (resultSet.next()) {
                    //itemDTO.setProdName(nameText.getText());
                    itemDTO.setDate(jDateChooser1.getDate().toString());
                    itemDTO.setQuantity(Integer.parseInt(quantityText.getText()));
                    //itemDTO.setCostPrice(Double.parseDouble(costText.getText()));
                    //itemDTO.setSellPrice(Double.parseDouble(sellText.getText()));
                    //itemDTO.setBrand(brandText.getText());
                    Double costPrice = Double.parseDouble(costText.getText());
                    Double totalCost = costPrice * Integer.parseInt(quantityText.getText());
                    itemDTO.setTotalCost(totalCost);

                    new ItemDAO().addBookingDAO(itemDTO);
                    loadDataSet();
                } else
                    JOptionPane.showMessageDialog(null, "This seems to be a new item" +
                            " that hasn't been added yet.\nPlease add this item in the \"Products\" section before proceeding.");
    ...
    private void deleteButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteButtonActionPerformed
        if (purchaseTable.getSelectedRow()<0)
            ...
            if(opt==JOptionPane.YES_OPTION) {
                new ItemDAO().deleteBookingDAO((int) purchaseTable.getValueAt(purchaseTable.getSelectedRow(),0));
                new ItemDAO().editBookedStock(itemCode, quantity);
                loadDataSet();
            }
        }
    ...
    private void codeTextKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_codeTextKeyReleased
        try {
            ResultSet resultSet = new ItemDAO().getItemFromCode(codeText.getText());
            if (resultSet.next()) {
                nameText.setText(resultSet.getString("itemname"));
    ...
    public void loadDataSet() {
        try {
            ...
            purchaseTable.setModel(itemDAO.buildTableModel(itemDAO.getItemInfo()));
